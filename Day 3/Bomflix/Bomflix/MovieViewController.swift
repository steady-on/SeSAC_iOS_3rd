...
class MovieViewController: UIViewController {
    ...
    func pickRandomMoviesUp() {
        pickedContents = []
        
        let movies = ["1", "2", "3", "4", "5", "극한직업", "도둑들", "명량", "베테랑", "부산행", "신과함께인과연", "신과함께죄와벌", "아바타", "알라딘", "암살", "어벤져스엔드게임", "왕의남자", "태극기휘날리며", "택시운전사", "해운대"]
        
        while pickedContents.count < 4 {
            let pick = movies.randomElement()!
            pickedContents.insert(pick)
        }
    }
    
    func setImagesToUIImages() {
        let picked = Array(pickedContents)
        mainContentsImage.image = UIImage(named: picked[0])
        firstRecommandContents.image = UIImage(named: picked[1])
        secondRecommandContents.image = UIImage(named: picked[2])
        thirdRecommandContents.image = UIImage(named: picked[3])
    }
